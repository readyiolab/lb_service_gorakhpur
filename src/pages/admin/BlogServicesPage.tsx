...
export default function BlogServicesPage() {
  ...
  const fetchBlogs = async () => {
    try {
      const data = await blogApi.getAll('lb_services', { limit: 100 });
  ...
  const handleEditClick = (blogId: number) => {
    navigate(`/admin/blog/services/edit/${blogId}`);
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">LB Services Blog</h1>
          <p className="text-muted-foreground">Manage blog posts for LB Services</p>
        </div>
        <Button onClick={() => navigate('/admin/blog/services/create')}>
