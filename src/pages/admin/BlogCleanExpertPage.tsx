...
export default function BlogCleanExpertPage() {
  ...
  const fetchBlogs = async () => {
    try {
      const data = await blogApi.getAll('clean_expert', { limit: 100 });
  ...
  const handleDeleteConfirm = async () => {
    ...
    setDeleting(true);
    try {
      const blogToDeleteObj = blogs.find((blog) => blog.blog_id === blogToDelete);
      if (!blogToDeleteObj) {
        toast({
          title: 'Error',
      ...
      const result = await blogApi.delete(blogToDelete, 'clean_expert');
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Clean Expert Blog</h1>
          <p className="text-muted-foreground">Manage blog posts for Clean Expert</p>
        </div>
        <Button onClick={() => navigate('/admin/blog/clean-expert/create')}>
                    ...
                    <TableCell>
                      <div className="flex gap-2">
                        <Button
                          variant="outline"
                          size="sm"
                          onClick={() => navigate(`/admin/blog/clean-expert/edit/${blog.blog_id}`)}
                        >
                          Edit
                        </Button>
                        <Button
