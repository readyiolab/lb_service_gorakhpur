...
export type BlogSite = 'lb_services' | 'lb_interiors' | 'clean_expert';
...
export const blogApi = {
  create: async (formData: FormData, site: BlogSite) => {
  ...
  update: async (blogId: number, formData: FormData, site: BlogSite) => {
    formData.append('blog_site', site);
  ...
  delete: async (blogId: number, site: BlogSite) => {
  ...
  getById: async (blogId: number, site: BlogSite) => {
    const response = await fetch(`${API_BASE_URL}/blog/id/${blogId}?blog_site=${site}`, {
      headers: {
        'Authorization': `Bearer ${authApi.getToken()}`,
      },
    });
  ...
  getAll: async (site?: BlogSite, options?: { page?: number; limit?: number; status?: string }) => {
    const params = new URLSearchParams();
    if (site) params.set('blog_site', site);
    if (options?.page) params.set('page', String(options.page));
    if (options?.limit) params.set('limit', String(options.limit));
    if (options?.status) params.set('blog_status', options.status);

    const url = `${API_BASE_URL}/blog${params.toString() ? `?${params.toString()}` : ''}`;
    const response = await fetch(url, {
      headers: {
        'Authorization': `Bearer ${authApi.getToken()}`,
      },
  ...
  getBySlug: async (slug: string, site: BlogSite = 'lb_services') => {
