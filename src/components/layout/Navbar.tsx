...
const Navbar = () => {
  ...
  return (
    <header className="sticky top-0 z-50 bg-card/95 backdrop-blur-sm border-b border-border">
          ...
          <div className="hidden lg:flex items-center gap-8">
            {navLinks.map((link) => (
              <Link
                key={link.path}
                to={link.path}
                className={`text-sm font-medium transition-colors hover:text-orange-600 ${isActive(link.path) ? "text-orange-600" : "text-foreground"
                  }`}
              >
                {link.name}
              </Link>
      ...
      <div
        className={`fixed top-0 left-0 h-screen w-screen bg-card z-50 lg:hidden transform transition-transform duration-300 ease-in-out ${isOpen ? "translate-x-0" : "-translate-x-full"
          }`}
      >
          ...
          <div className="flex flex-col gap-2 p-6 flex-1">
            {navLinks.map((link) => (
              <Link
                ...
                to={link.path}
                onClick={() => setIsOpen(false)}
                className={`px-6 py-4 rounded-lg text-base font-medium transition-all ${isActive(link.path)
